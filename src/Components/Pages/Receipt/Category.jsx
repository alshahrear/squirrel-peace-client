...
const Category = () => {
  ...
  const handleSubmit = async () => {
    try {
      if (!formData.name) return toast.error("ক্যাটাগরির নাম লিখুন");
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      text: "এই ক্যাটাগরি ডিলিট করা হবে!",
      icon: "warning",
        ...
        <div ref={formRef} className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row gap-3 items-end">
            <div className="w-full">
              <label className="text-[10px] font-black text-slate-500 uppercase mb-1.5 block ml-1">Category Name</label>
              <input
        ...
        <div className="space-y-3">
          {currentCategories.length === 0 ? (
            <div className="flex flex-col items-center py-12 text-slate-300">
              ...
              <p className="text-xs font-black uppercase tracking-widest text-slate-400">No categories found</p>
            </div>
          ) : (
