...
const StoryDown = () => {
    const features = [
        {
            icon: <FaCoffee className="text-3xl sm:text-4xl mb-2" />,
            title: "Awesome Aroma",
            description: "You will definitely be a fan of the design & aroma of your coffee"
        },
        {
            icon: <MdHighQuality className="text-3xl sm:text-4xl mb-2" />,
            title: "High Quality",
            description: "We served the coffee to you maintaining the best quality"
        },
        {
            icon: <GiCoffeeBeans className="text-3xl sm:text-4xl mb-2" />,
            title: "Pure Grades",
            ...
            <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 text-center text-[#3c2a21]">
                {features.map((feature, index) => (
                    <div key={index} className="px-2 sm:px-4">
                        ...
                        <h3 className="text-lg sm:text-lg font-semibold mt-2">{feature.title}</h3>
                        <p className=" sm:text-sm mt-1 leading-relaxed">{feature.description}</p>
                    </div>
                ))}
            </div>
        </div>
