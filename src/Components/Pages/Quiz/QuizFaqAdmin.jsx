  ...
  return (
    <div className="my-12 max-w-screen-xl mx-auto px-2 sm:px-4">
      ...
      <div className="mb-4 text-right pr-2 sm:pr-3 text-lg sm:text-2xl font-semibold text-gray-800">
        Total Questions: <span className="text-[#2acb35]">{quizFaqs.length}</span>
      </div>
      ...
      <div className="overflow-x-auto shadow-md rounded-lg">
        <table className="table-auto w-full text-left text-sm sm:text-base border-collapse faq-table">
          ...
          <tbody className="divide-y divide-gray-200">
            {quizFaqs.length > 0 ? (
              quizFaqs.map((faq, index) => (
                <FaqQus
                  key={faq._id}
                  faq={faq}
                  index={index}
                  quizFaqs={quizFaqs}
                  setQuizFaqs={setQuizFaqs}
      ...
      {showModal && (
        <dialog open className="modal">
          <div className="modal-box relative max-w-lg">
            ...
            <p className="text-gray-700 text-lg whitespace-pre-line">{selectedQuestion}</p>
          </div>
        </dialog>
      )}
      ...
      <style>{`
        ...
        @media (max-width: 1024px) {
          ...
          .faq-table tbody tr td.text-center { padding-left: 0; }
          .faq-table tbody tr td.text-center button { margin: 0 auto; display: block; }
        }
      `}</style>
    </div>
  );
...
const FaqQus = ({ faq, index, quizFaqs, setQuizFaqs, openModalWithQuestion }) => {
  ...
  const handleFaqDelete = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://squirrel-peace-server.onrender.com/quizFaqs/${_id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
            if (data.deletedCount > 0) {
              const remainingFaqs = quizFaqs.filter((item) => item._id !== _id);
              setQuizFaqs(remainingFaqs);

              Swal.fire({
                position: "top-end",
                icon: "success",
                title: `${name}'s question has been deleted`,
  ...
  return (
    <tr className="odd:bg-white even:bg-gray-100 transition-all duration-200">
      ...
      <td className="py-3 px-2 sm:px-4 text-center" data-label="Actions">
        <button
